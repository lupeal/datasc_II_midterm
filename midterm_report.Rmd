...
We have 15 variables, including our response (recovery time). We calculated summary statistics for 14 of them, grouping them by study group, since we are interested in kmowing whether the two study groups are similar. We noticed that they are very similar in all our proposed covariates, yet the mean recovery time for individuals in gorup B is five days more than for individuals in group A. It is also important to note that group A has twice as many individuals as group B.
...
featurePlot(x[, c(1, 8:10, 13,14)], y, plot = 'scatter', 
            labels = c('', 'Y'), type = c('p'), caption = "dvjnv")
```
